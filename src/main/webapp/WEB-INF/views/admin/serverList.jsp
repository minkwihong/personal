					...
					<div class="col-md-12">
						<div class="box">
							<div class="box-header with-border">
								<h3 class="box-title">KSignAccess Server List</h3>
							</div>
							<div class="box-body">
							<%--<form class="form-search" id="f">--%>
									<input type="hidden" id="curPage" value="1">
									<input type="hidden" id="sortType" value="serverName" data-sorttype="desc">

									<div class="row">
										<div class="col-xs-3 col-sm-3">
											<div class="input-group input-group-sm">
												<input type="text" class="form-control" id="s_value" name="s_value" placeholder="serverName">
								...
								<table class="table table-striped table-bordered table-hover" id="listTable">
									<thead>
										<tr>
											<th>#</th>
											<th>serverName <i class="fa fa-sort-desc" id="userIdSort" data-value="desc"></i></th>
											<th>type</th>
											<th>ip</th>
											<th>port</th>
											<th>url</th>
	...
	<script>
		...
		function initSucess(result) {
			...
			for (var i = 0; i < objList.length; i++) {
				...
				var userId = _highlight(_omit(objList[i].serverName, 40), "", "orange");
				var type = isNull(objList[i].type);
				var a_link = "serverModifyView.do?x="
						+ encodeURI(encodeURIComponent(objList[i].serverName));

				var t_row = "<tr>\n" + "  <td>" + (idx_b--) + "</td>\n" +

				"  <td><a href='" + a_link + "'>" + objList[i].serverName	+ "</a></td>\n" +
				"  <td>" + isNull(objList[i].type) + "</td>\n" +
				"  <td>" + isNull(objList[i].ip)	+ "</td>\n" +
				"  <td>" + isNull(objList[i].port) + "</td>\n" +
				"  <td>" + isNull(objList[i].url) + "</td>\n" +
				"  <td>" + isNull(objList[i].gid) + "</td>\n" +
				"  <td>" + isNull(objList[i].certdn) + "</td>\n" +
                "  <td>" + isNull(objList[i].usableAgent) + "</td>\n"
		...
		function registView() {
			var a_link = "serverRegistView.do";
