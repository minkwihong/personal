...
//-----------------------------------------------------------------------------
function handlePost(subUri, paramObj, callback_success, callback_error, is_ga) {
    $.ajax({
    	type: "POST",
		...
		success: function(result, status, xhr) {
	         ...
	         if(_is_ie_789 && result && result.code && result.message && result.code != 200 && result.code != 205) {
	             ...
	             if(json.code == '411' || json.code == '451') {
	                 location.reload();
	            		   ...
	            		   json.code == '446' || json.code == '447' || json.code == '448') {
	                 alert(json.message);
	         ...
	         if(typeof(_ga_send) == 'function') _ga_send(subUri, is_ga);
...
function handlePatch(subUri, paramObj, callback_success, callback_error, is_ga) {
    $.ajax({
    	type: "PATCH",
    	url: x_apiUrl + subUri + (_is_ie_789 ? "?callback=?&" : "?") + "apiKey=" + x_apiKey,
    	data: (_is_ie_789 ? encodeURIComponent(JSON.stringify(paramObj)) : JSON.stringify(paramObj)),
...
function handleDelete(subUri, paramObj, callback_success, callback_error, is_ga) {
    $.ajax({
    	type: "DELETE",
    	url: x_apiUrl + subUri + (_is_ie_789 ? "?callback=?&" : "?") + "apiKey=" + x_apiKey,
    	data: (_is_ie_789 ? encodeURIComponent(JSON.stringify(paramObj)) : JSON.stringify(paramObj)),
...
function handleGet(subUri, paramObj, callback_success, callback_error, is_ga) {
    $.ajax({
    	type: "GET",
    	url: x_apiUrl + subUri + (_is_ie_789 ? "?callback=?&" : "?") + "apiKey=" + x_apiKey,
    	data: paramObj,
		...
		success: function(result, status, xhr) {
	         ...
	         if(_is_ie_789 && result && result.code && result.message && result.code != 200) {
	             ...
	             if(json.code == '411' || json.code == '451') {
	                 location.reload();
	             } else if(json.code == '446' || json.code == '447' || json.code == '448') {
	                 alert(json.message);    
	             } else {
	             	//alert(subUri + " : [ERR." + json.code + "] " + json.message);
	             	alert("[" + json.code + "] " + json.message);
        ...
        beforeSend: function(xhr, settings) {
    		$("#progress").attr("visibility","visible");
        },
        complete: function(xhr, textStatus) {
            $("#progress").attr("visibility","hidden");
